<template>
  <div>
    <el-table :data="table_data" stripe class="goods_list" ref="goods_list" :header-cell-style="{'text-align': 'center'}" :cell-style="{'text-align': 'center'}">
      ...
      <el-table-column prop="qty" label="显示数量" >
      </el-table-column>
...
export default {
  ...
  methods: {
    handleEdit(index, row) {
			this.$activationNav('/Module/Add')
      this.$router.push({ path: `/module/Editor/${row.id}` })
    },
    ...
    putData(index, name) {
      ...
      params.data = data
      this.$http(this.$api.Module,'update',params).then( res => {
        if (name == 'sort') {
    ...
    deleteData(index, row) { // 这里的row还是双向绑定的
      let id = row.id;
      row.id = false;
      this.$http(this.$api.Module,'delete',{id:id}).then( res => {
    ...
    getData() {
      ...
      this.$http(this.$api.Module,'index',params).then( res => {
