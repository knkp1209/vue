...
<script>
	// Import TinyMCE
	import axios from 'axios'; // Todo:: 应该可优化绑定 Vue 中
			...
			const setting = {
				...
				images_upload_handler: function (blobInfo, success, failure) {
					...
					function uploadPic() {
						...
						axios({url: _this.$config.BaseUrl + _this.$api.Upload, method: 'POST', data: formData}).then(data => {
							success(_this.$config.StaticResourceUrl + data.data.data.url)// 返回图片地址
						}).catch(error => {
							failure(error)
						})
					}
