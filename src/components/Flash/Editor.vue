  ...
  <div class="main">
    <el-form label-position="right" label-width="80px">
      ...
      <el-form-item label="秒杀价">
        <el-input v-model="sell_price" name="sell_price" v-validate="'decimal:2|min_value:0|max_value:99999999|required'" placeholder="秒杀价格 (必填)"></el-input>
        <i class="my_err" v-show="errors.has('sell_price')">{{errors.first('sell_price')}}</i>
      </el-form-item>
      <el-form-item label="秒杀库存">
        <el-input v-model="stock" name="stock" v-validate="'numeric|min_value:0|max_value:99999999|required'" placeholder="秒杀库存(必填)"></el-input>
        <i class="my_err" v-show="errors.has('stock')">{{errors.first('stock')}}</i>
      </el-form-item>
      <el-form-item label="限购数">
        <el-input v-model="max_qty" name="max_qty" v-validate="'numeric|min_value:0|max_value:99999999'" placeholder="每次下单限购数(不填为不限购)"></el-input>
        <i class="my_err" v-show="errors.has('max_qty')">{{errors.first('max_qty')}}</i>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getProductList() {
      this.$http(this.$api.Flash,'create').then( res => {
    ...
    getProduct() {
      this.$http(this.$api.Flash,'edit',{id:this.id}).then( res => {
        let { msg, result } = res.data
    ...
    post() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.$http(this.$api.Flash,type,params).then( res => {
            this.$message({
              message: res.data.msg,
              type: 'success',
              onClose: () => {
                this.$router.push({ path: '/Flash/List' })
              }
            });
          }).catch( msg => {
            this.$message.error(msg)
