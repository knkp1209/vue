<template>
  <div>
    <el-table :data="table_data" stripe class="goods_list" ref="goods_list" :header-cell-style="{'text-align': 'center'}" :cell-style="{'text-align': 'center'}">
      ...
      <el-table-column prop="total" label="可领取总次数" >
      </el-table-column>
      <el-table-column prop="use_total" label="每次领取张数" >
      </el-table-column>
      <el-table-column prop="stock" label="剩余领取次数" >
      </el-table-column>
      <el-table-column prop="amount" label="优惠金额" >
      </el-table-column>
      <el-table-column prop="amount_limit" label="限制使用金额" >
      </el-table-column>
      <el-table-column label="状态" width="350px">
        <template slot-scope="scope">
          ...
          <div v-else-if="scope.row.enable">
            <el-tag type='danger'>可领取</el-tag>
          </div>
          <div v-else>
            <el-tag type='danger'>不可领取</el-tag>
          </div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    handleEdit(index, row) {
			this.$activationNav('/Coupon/Add')
      this.$router.push({ path: `/coupon/Editor/${row.id}` })
    },
    ...
    putData(index, name) {
      ...
      params.data = data
      this.$http(this.$api.Coupon,'update',params).then( res => {
        if (name == 'sort') {
    ...
    deleteData(index, row) { // 这里的row还是双向绑定的
      let id = row.id;
      row.id = false;
      this.$http(this.$api.Coupon,'delete',{id:id}).then( res => {
    ...
    getData() {
      ...
      this.$http(this.$api.Coupon,'index',params).then( res => {
