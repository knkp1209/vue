<template>
  <div>
    <el-table :data="table_data" stripe class="goods_list" ref="goods_list" :header-cell-style="{'text-align': 'center'}" :cell-style="{'text-align': 'center'}">
      ...
      <el-table-column prop="sell_price" label="团购价格" >
      </el-table-column>
      <el-table-column prop="people_qty" label="开团人数">
      </el-table-column>
...
export default {
  ...
  methods: {
    handleEdit(index, row) {
    	this.$activationNav('/Team/Add')
      this.$router.push({ path: `/Team/Editor/${row.id}` })
    },
    ...
    putData(index, name) {
      ...
      this.$http(this.$api.Team,'update',params).then( res => {
        if (name == 'sort') {
    ...
    deleteData(index, row) { // 这里的row还是双向绑定的
      let id = row.id;
      row.id = false;
      this.$http(this.$api.Team,'delete',{id:id}).then( res => {
        this.$message.success(res.data.msg);
    ...
    getData() {
      ...
      this.$http(this.$api.Team,'index',params).then( res => {
